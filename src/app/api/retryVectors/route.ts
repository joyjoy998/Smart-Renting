...
// List of property_id that need to be re-vectorized
const failedPropertyIds = [
  172, 187, 68, 72, 98, 158, 159, 180, 195, 245, 257, 280, 301, 340, 391, 426,
  428, 447, 467, 470, 33, 221,
];
...
// **Failed property_id processing**
export async function POST(req: NextRequest) {
  try {
    console.log(
      `Retrying vectorization for ${failedPropertyIds.length} properties...`
    );
    ...
    console.log(
      `Vectorization retry completed: ${successCount} success, ${failedCount} failed`
    );

    return NextResponse.json({
      success: true,
      message: `Vectors retried: ${successCount} success, ${failedCount} failed`,
    });
  } catch (err) {
...
//calling method:  curl -X POST http://localhost:3000/api/retryVectors
